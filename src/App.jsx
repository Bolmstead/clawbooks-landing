      ...
      <div className="container hero-inner">
        <div className="hero-text">
          ...
          <p className="hero-sub">
            ClawBooks watches your inbox, reads every invoice with AI, and posts to QuickBooks —
            you just tap <strong>Approve</strong> on Telegram.
          </p>
...
function Features() {
  ...
  const features = [
    { icon: '📧', title: 'Email Monitoring', desc: 'Watches your inbox 24/7 for invoices, bills, and receipts. iCloud, Gmail, or any IMAP account.' },
    { icon: '🤖', title: 'AI Extraction', desc: 'Reads every invoice and extracts vendor, amount, line items, and due dates automatically.' },
    { icon: '✅', title: 'Human Approval', desc: 'Get a Telegram message before anything posts. Review and approve or reject in seconds. You stay in control.' },
    { icon: '📊', title: 'QuickBooks Sync', desc: 'Approved transactions post directly to your QB account with full audit log and idempotency protection.' },
  ]
  return (
    <section className="section" id="features" ref={ref}>
      <div className="container">
        <div className={`section-header fade-in ${inView ? 'visible' : ''}`}>
          ...
          <p className="section-sub">From inbox to QuickBooks in seconds, with you approving every step.</p>
        </div>
        <div className="features-grid">
          {features.map((f, i) => (
            <div key={f.title} className={`feature-card fade-in ${inView ? 'visible' : ''}`} style={{ transitionDelay: `${i * 0.1}s` }}>
...
function HowItWorks() {
  const [ref, inView] = useInView()
  const steps = [
    { n: '01', title: 'Invoice arrives in your email', desc: 'ClawBooks monitors your connected inbox and detects new invoices automatically.' },
    { n: '02', title: 'AI reads and proposes a transaction', desc: 'Our AI extracts all fields and builds a QuickBooks transaction with confidence scoring.' },
    { n: '03', title: 'You approve on Telegram in seconds', desc: 'Tap Approve and it posts instantly. Tap Reject and it\'s discarded. Full audit trail either way.' },
  ]
  return (
    <section className="section section-dark" id="how-it-works" ref={ref}>
      <div className="container">
        ...
        <div className="steps">
          {steps.map((s, i) => (
            <div key={s.n} className={`step fade-in ${inView ? 'visible' : ''}`} style={{ transitionDelay: `${i * 0.15}s` }}>
    ...
    <section className="section" ref={ref}>
      <div className="container mockup-section">
        <div className={`mockup-text fade-in ${inView ? 'visible' : ''}`}>
          ...
          <p>No dashboards to log into. No emails to reply to. Just a tap, and it's done.</p>
        ...
        <div className={`phone-wrap fade-in ${inView ? 'visible' : ''}`} style={{ transitionDelay: '0.2s' }}>
          <div className="phone">
            ...
            <div className="phone-screen">
              ...
              <div className="tg-messages">
                <div className="tg-msg">
                  ...
                  <div className="tg-msg-row"><span>From</span><strong>Acme Design Co.</strong></div>
                  <div className="tg-msg-row"><span>Amount</span><strong className="accent">$2,450.00</strong></div>
                  <div className="tg-msg-row"><span>Category</span><strong>Web Design</strong></div>
                  <div className="tg-msg-row"><span>Due</span><strong>Mar 1, 2026</strong></div>
                  {approved === null && (
                    <div className="tg-btns">
                      <button className="tg-approve" onClick={() => setApproved(true)}>✓ Approve</button>
                      <button className="tg-reject" onClick={() => setApproved(false)}>✕ Reject</button>
                    </div>
                  )}
                  {approved === true && <div className="tg-confirmed">✅ Posted to QuickBooks!</div>}
                  {approved === false && <div className="tg-rejected">❌ Invoice rejected</div>}
                </div>
              </div>
            </div>
          </div>
    ...
    <section className="section cta-section" id="cta" ref={ref}>
      ...
      <div className={`container cta-inner fade-in ${inView ? 'visible' : ''}`}>
        ...
        {!submitted ? (
          ...
          <div className="cta-thanks">🎉 You're on the list! We'll be in touch soon.</div>
        )}
      </div>
    </section>
  )
